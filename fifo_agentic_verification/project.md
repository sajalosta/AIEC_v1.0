...
No checker code was machine generated!
...
The agent's reasoning and decision points: Agent builds the design model, stores tests and RULES into RAG, retrieve the test from RAG and genrate it, run the test and collect the logs, checker retreives the RULE files from the RAG and compares with log file of test run and comes with VERDICT PASS/FAIL, then RAGAS judges the retrievals and the TEST VERDICT of the checker. Finally the report prints the report of the passed/failed tests.


What tools the agent calls and why: Agent calls agents as tools, it can all tols to generate tests/ run tests/ build design models/ call RAGAS To do evals on retreival and test checker output
...
Your own personal data, uploaded to your application (e.g., RAG): I have my own data in the docs folder (Test plan and rule file)
The ability to search publicly available data (e.g., a simple agentic search tool like Tavily) : Dont need this for the project.
...
Deploy your prototype to public endpoint using a tool like Vercel, Render, or FastAPI Cloud
